<?php 
	trait SearchModel{
		...
		//ham lay danh sach cac ban ghi co phan trang
		public function modelReadSearchProductPrice($range,$recordPerPage){
			//lay bien p truyen tu url
			$p = isset($_GET["p"]) && is_numeric($_GET["p"]) && $_GET["p"] > 0 ? ($_GET["p"]-1) : 0;			
			//lay tu ban ghi nao
			$from = $p * $recordPerPage;
			...
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->query("select * from products where price <= $range order by id desc limit $from,$recordPerPage");
			//lay toan bo ket qua tra ve
			$result = $query->fetchAll();			
			//---
			return $result;
		...
		//tinh tong so ban ghi
		public function totalRecordSearchProductPrice($range,$recordPerPage){
			//lay bien ket noi
			$conn = Connection::getInstance();
			$query = $conn->query("select id from products where price <= $range");
			//tra ve tong so ban ghi
			return $query->rowCount();
		}	
		//smart search
